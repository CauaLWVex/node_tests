...
app.get('/analyze', async (req, res) => {
    try {
        ...
        const [axeAnalyze, scoreAnalyze] = await Promise.all([analyzePage(url), analyzeScore(url)])
        if (axeAnalyze == null || scoreAnalyze == null) {
            throw new Error('Error during the analyze')
        }
        axeAnalyze.score = scoreAnalyze
        res.json(axeAnalyze)
    } catch (e) {
        console.log(e)
        return res.status(500).send('Ocorreu um erro ao analisar a página, por favor tente novamente')
    }
})
...
const analyzePage = async (url) => {
    ...
    try {
        ...
        const results = await new AxePuppeteer(page).configure({
            locale: locale,
        }).options({ ancestry: true }).analyze();
...
const analyzeScore = async (url) => {
    const browser = await puppeteer.launch({ headless: 'new', args: ['--show-paint-rects'] })
    try {
        const endpoint = browser.wsEndpoint();
        const endpointURL = new URL(endpoint)
        const reportData = await lighthouse(
            url,
            Object.assign({}, {
                port: endpointURL.port,
            }, { output: 'json', onlyCategories: ['accessibility'] })
        )
        const report = JSON.parse(reportData.report)
